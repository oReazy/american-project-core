...
async def Show1(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
    await database.setUserData(message.from_id, 'state', "'excursion.Show1'")
    await message.answer(
        message=f"👋 Добро пожаловать на @{DATA_SETTINGS[3]}({DATA_SETTINGS[1]}) в штат @{DATA_SETTINGS[4]}({DATA_SETTINGS[2]})\n\n"
                ...
                f"Новостная группа ВКонтакте » @{DATA_SETTINGS[3]}({DATA_SETTINGS[1]})\n"
                f"Новостная группа сервера » @{DATA_SETTINGS[4]}({DATA_SETTINGS[2]})",
        keyboard=(
...
async def Show2(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def Show3(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def Show4(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def Show5(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def Show6(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def Show7(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def Show8(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def Show9(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def Show10(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def Show11(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
    await database.setUserData(message.from_id, 'state', "'excursion.Show10'")
    await message.answer(
