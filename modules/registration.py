...
async def registration_1(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def registration_1_check(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
    if 3 <= len(message.text) <= 15:
        ...
        if await database.findBaseData('nick', f"'{message.text}'") == 0:
            ...
            await registration_2(message, bot, api, DATA_USER, DATA_SETTINGS)
        else:
            await message.answer(
                message='❌ Ошибка. Данный ник уже занят. Попробуйте другой'
            )
            await registration_1(message, bot, api, DATA_USER, DATA_SETTINGS)
    else:
        await message.answer(
...
async def registration_2(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
    await database.setUserData(message.from_id, 'state', "'registration.registration_2'")
    await message.answer(
...
# Выбор пола персонажа
async def registration_2_man(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
    await database.setMultiUserData(message.from_id, "sex = 'Мужчина', state = 'registration.registration_3'")
    await registration_3(message, bot, api, DATA_USER, DATA_SETTINGS)

# Выбор пола персонажа
async def registration_2_woman(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def registration_3(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
    await database.setUserData(message.from_id, 'state', "'registration.registration_3'")
    await message.answer(
        message=f"⤵ Выберите национальность вашему персонажу",
        keyboard=(
            Keyboard(one_time=True, inline=False)
...
async def registration_3_check(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
    await database.setMultiUserData(message.from_id, f"nationality = '{message.text}', state = 'registration.registration_4'")
    await registration_4(message, bot, api, DATA_USER, DATA_SETTINGS)

# ——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————

# Просьба ввести возраст персонажа
async def registration_4(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def registration_4_check(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
    if message.text.isdigit():
        age = int(message.text)
        if 18 <= age <= 70:
            await database.setMultiUserData(message.from_id, f"age = '{age}', state = 'registration.registration_5'")
            await registration_5(message, bot, api, DATA_USER, DATA_SETTINGS)
        else:
            await message.answer(
...
async def registration_5(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def registration_5_friend(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
    update = DATA_SETTINGS[9] + 1
    await database.setBdData('settings', 'id', "'1'", 'statistics_friend', f"'{update}'")
    await registration_6(message, bot, api, DATA_USER, DATA_SETTINGS)



async def registration_5_list_chatbot(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def registration_5_search(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def registration_5_youtube(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def registration_5_other(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def registration_6(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def registration_6_accept(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
    ...
    await registration_7(message, bot, api, DATA_USER, DATA_SETTINGS)



async def registration_6_denial(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def registration_7(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
    await database.setMultiUserData(message.from_id, f"lvl = '{DATA_SETTINGS[18]}', dollars = '{DATA_SETTINGS[17]}', donate = '{DATA_SETTINGS[19]}'")
    await registration_8(message, bot, api, DATA_USER, DATA_SETTINGS)

# ——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————

# Забрать пособие и начать игру
async def registration_8(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def registration_9(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def registration_9_1(from_id, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
...
async def newAccaunt(message: Message, bot: Bot, api: API, DATA_USER, DATA_SETTINGS):
